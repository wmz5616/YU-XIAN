  ...
  <div class="container mx-auto px-4 py-8 max-w-4xl">
    ...
    <div v-if="store.items.length > 0" class="flex flex-col lg:flex-row gap-8">
      ...
      <div class="flex-1 bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <ul class="divide-y divide-slate-100">
          <li v-for="item in store.items" :key="item.id" class="p-6 flex items-center gap-4 hover:bg-slate-50 transition">
            ...
            <div class="flex-1">
              ...
              <div class="flex justify-between items-center">
                ...
                <div class="flex items-center bg-white border border-slate-200 rounded h-8">
                  <button @click="store.decreaseItem(item.id)" class="w-8 h-full flex items-center justify-center text-slate-500 hover:bg-slate-50 disabled:opacity-30">-</button>
                  <span class="text-xs font-bold w-8 text-center">{{ item.quantity }}</span>
                  <button @click="store.addToCart(item)" class="w-8 h-full flex items-center justify-center text-slate-500 hover:bg-slate-50" :disabled="item.quantity >= item.stock">+</button>
                </div>
              </div>
            </div>
          </li>
      ...
      <div class="w-full lg:w-80">
        <div class="bg-white rounded-xl shadow-lg border border-slate-100 p-6 sticky top-24">
          ...
          <div class="border-t border-slate-100 pt-4 flex justify-between items-end mb-6">
            ...
            <span class="text-3xl font-bold text-blue-900">¥{{ store.totalPrice }}</span>
          </div>
    ...
    <div v-else class="flex flex-col items-center justify-center py-24">
      <div class="relative w-40 h-40 mb-6 flex items-center justify-center">
        <div class="absolute inset-0 border-2 border-dashed border-slate-200 rounded-full animate-[spin_10s_linear_infinite]"></div>
      ...
      <button @click="router.push('/')" class="group relative px-8 py-3 bg-blue-900 text-white rounded-full font-bold shadow-lg transition overflow-hidden">
